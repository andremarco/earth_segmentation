    ...
    def __init__(
        ...
        if deep_stem:
            self.conv1 = nn.Sequential(
                nn.Conv2d(input_size, 64, 3, 2, 1, bias=False),
                norm_layer(64),
                nn.ReLU(True),
            ...
            self.conv1 = nn.Conv2d(input_size, 64, 7, 2, 3, bias=False)
...
def resnet50_v1s(input_size=3, pretrained=False, root="~/.torch/models", **kwargs):
    model = ResNetV1b(BottleneckV1b, [3, 4, 6, 3], input_size, deep_stem=not pretrained, **kwargs)
    if pretrained:
        print("Loading weights...")
        model.load_state_dict(
            torch.hub.load_state_dict_from_url('https://download.pytorch.org/models/resnet50-19c8e357.pth'),
            strict=False,
        )
